...
namespace Cityware\Communication\Rest;
...
class Server2 {
        ...
        if (isset($request['method'])) {
            ...
            if (isset($this->_functions[$this->_method])) {
                if ($this->_functions[$this->_method] instanceof
                        ReflectionFunction || $this->_functions[$this->_method] instanceof
                        ReflectionMethod && $this->_functions[$this->_method]->isPublic()
                ) {
                    ...
                    if (count($callingArgs) < count($funcArgs)) {
                        $result = $this->fault(new Exception('Invalid Method Call to ' . $this->_method . '. Missing argument(s): ' . implode(', ', $missingArgs) . '.'), 400);
                    }
                    if (!$result && $this->_functions[$this->_method] instanceof ReflectionMethod) {
                        // Get class
                        ...
                        if ($this->_functions[$this->_method]->isStatic()) {
                            ...
                            // Object method
                            $result = $this->_callObjectMethod($class, $callingArgs);
                        }
                    } elseif (!$result) {
                        try {
                            $result = call_user_func_array($this->_functions[$this->_method]->getName(), $callingArgs);
                        } catch (Exception $e) {
                            $result = $this->fault($e);
                        }
                    }
                } else {
                    $result = $this->fault(new Exception("Unknown Method '$this->_method'."), 404);
                }
            } else {
                $result = $this->fault(new Exception("Unknown Method '$this->_method'."), 404);
            }
        } else {
            $result = $this->fault(new Exception("No Method Specified."), 404);
        }
    ...
    public function fault($exception = null, $code = null) {
        ...
        if ($exception instanceof Exception) {
            ...
            $xmlResponse->appendChild(
                    $dom->createElement('message', 'An unknown error occured. Please try again.')
            );
        } else {
            $xmlResponse->appendChild(
                    $dom->createElement('message', 'Call to ' . $method . ' failed.')
            );
        }
        $xmlMethod->appendChild($xmlResponse);
    ...
    public function addFunction($function, $namespace = '') {
        ...
        foreach ($function as $func) {
            if (is_callable($func) && !in_array($func, self::$magicMethods)) {
                ...
                throw new Exception("Invalid Method Added to Service.");
            }
        }
    }
    ...
    protected function _callStaticMethod($class, array $args) {
        try {
            $result = call_user_func_array(
                    array($class, $this->_functions[$this->_method]->getName()), $args
            );
        } catch (Exception $e) {
            $result = $this->fault($e);
        }
    ...
    protected function _callObjectMethod($class, array $args) {
        try {
            ...
            throw new Exception('Error instantiating class ' . $class . ' to invoke method ' . $this->_functions[$this->_method]->getName() . ' (' . $e->getMessage() . ') ', 500, $e);
        }
        try {
