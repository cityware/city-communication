...
class Server {
    ...
    public function unauthorized($ask = false) {
        ...
        throw new Exception("You are not authorized to access this resource.", 401);
    }
    ...
    protected function findUrl() {
        ...
        foreach ($urls as $url => $call) {
            ...
            if (!strstr($url, '$')) {
                ...
                $regex1 = preg_replace('/\\\\\$([\w\d]+)\.\.\./', '(?P<$1>.+)', str_replace('\.\.\.', '...', preg_quote($url)));
                $regex = preg_replace('/\\\\\$([\w\d]+)/', '(?P<$1>[^\/]+)', $regex1);
                $matches = null;
    ...
    public function getFormat() {
        ...
        if (isset(self::$formats[$override])) {
            $format = self::$formats[$override];
        } elseif (in_array(self::JSON, $accept)) {
            $format = self::JSON;
        }
        return $format;
    }
